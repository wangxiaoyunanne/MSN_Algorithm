  ...
  //  whole_path.pushback ( start_vertex );
    ...
    for(int i =0; i< path_length; i++)
    {
        ...
        int next_index =  (i+1) % path_length;
            ...
            if ( path[i].zCoord > path[ next_index ].zCoord +1 )
            {
                ...
                //    cout<< "add inner node";
                //    cout<< inner_v.xCoord<<inner_v.yCoord<<inner_v.zCoord;
                }
            }
            else if (path[i].zCoord < path[ next_index ].zCoord -1 )
            {
                for (int z = path[i].zCoord +1; z < path[next_index].zCoord; z++ )
                {
                    Vertex2 inner_v;
...
} // deAlloc3D for deleting degTable

void deAlloc_possi_pairs (vector<int> ** possi_pairs  )
{
    int i,j;
    for (i =0; i <XDIM +1 ; i++)
    {
        for (j=0;j < YDIM +1 ; j++ )
        {
            possi_pairs[i][j].clear();
        }
        delete [ ] possi_pairs[i];
    }
    delete [] possi_pairs;
}// deAlloc_possi_pairs

void deAlloc_possi_connects (vector<VerticalPairs> ** possi_connect  )
{
    int i,j;
    for (i =0; i <XDIM +1 ; i++)
    {
        for (j=0;j < YDIM +1 ; j++ )
        {
            possi_connect[i][j].clear();
        }
        delete [ ] possi_connect[i];
    }
    delete [] possi_connect;
}// deAlloc_possi_connects
...
//cout<< vertices_list.size()<< vertical_edges.size()<< edges.size()<<possible_path.size()<<endl;
     ...
     //           cout<< "reducible ? "<< check_reducible(whole_path)<<endl;            
                if ( ! check_reducible(whole_path) && whole_path.size() == LENGTH )
                {
                 //   cout<<"possi path"<<endl;
                 //   PrintVertices(possible_path);
                 //   cout<<"whole path"<<endl;
                 //   PrintVertices(whole_path);
                    deAlloc3D(degTable);
                    deAlloc_possi_pairs(possible_pairs);
